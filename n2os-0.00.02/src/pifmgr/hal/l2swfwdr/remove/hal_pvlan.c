                     ...
                     enum hal_pvlan_type type)
{
  ...
  ret = hal_ioctl (IPIBR_SET_PVLAN_TYPE, (unsigned long)bridge_name, 
                   vid, type, 0 , 0);
                    ...
                    unsigned short pvid, int associate)
{
  ...
  ret = hal_ioctl (IPIBR_SET_PVLAN_ASSOCIATE, (unsigned long)bridge_name,
                   vid, pvid, associate, 0);
                         ...
                         enum hal_pvlan_port_mode mode)
{
  ...
  ret = hal_ioctl (IPIBR_SET_PVLAN_PORT_MODE, (unsigned long)bridge_name,
                   ifindex, mode, 0, 0);
                            ...
                            unsigned short pvid, int associate)
{
  ...
  ret = hal_ioctl (IPIBR_SET_PVLAN_HOST_ASSOCIATION,(unsigned long)bridge_name,
                    ifindex, vid, pvid, associate);
