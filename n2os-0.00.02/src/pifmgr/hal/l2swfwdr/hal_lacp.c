...
{
  int ret;
  ret = hal_ioctl (IPILACP_DEL_AGG, (long) name, 0, 0, 0, 0, 0);
  if (ret < 0)
    return -ret;
    /* return -errno; */
  return HAL_SUCCESS;
				                   ...
				                   char *port, unsigned int port_ifindex)
{
  ...
  ret = hal_ioctl (IPILACP_AGG_LINK, (long) name, (long)port, 0, 0, 0, 0);
  if (ret < 0)
    return -ret;
  return HAL_SUCCESS;
}
				                     ...
				                     char *port, unsigned int port_ifindex)
{
  ...
  ret = hal_ioctl (IPILACP_DEAGG_LINK, (long) name, (long)port, 0, 0, 0, 0);
  if (ret < 0)
    return -ret;
  return HAL_SUCCESS;
}
