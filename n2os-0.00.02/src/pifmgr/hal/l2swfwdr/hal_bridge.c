...
hal_bridge_delete (char *name)
{
  ...
  ret = hal_ioctl (IPIBR_DEL_BRIDGE, (unsigned long)str, 0, 0, 0, 0, 0);
  if (ret < 0)
    return -errno;
...
hal_bridge_change_vlan_type (char *name, int is_vlan_aware,u_int8_t type)
{
  ...
  ret = hal_ioctl (IPIBR_CHANGE_VLAN_TYPE, (unsigned long)str,
...
hal_bridge_disable_ageing(char *name)
{
  ...
  ret = hal_ioctl (IPIBR_DISABLE_AGEING, (unsigned long)name, 0, 0, 0, 0, 0);
...
int hal_l2_qos_get_num_coq (unsigned int *num_cosq)
{
  *num_cosq = 0;
  return HAL_SUCCESS;
}

int hal_l2_qos_set_num_coq (unsigned int num_cosq)
{
  return HAL_SUCCESS;
}
...
int
hal_uni_add (char *name, unsigned int ifindex, u_char uni_type, u_int16_t svid)
{
  int ret;

  ret = hal_ioctl (IPI_UNI_ADD, (unsigned long)name, (unsigned long)ifindex, (unsigned long)uni_type, (unsigned long)svid, 0, 0);
...
hal_igmp_snooping_enable (char* bridge_name)
{
  ...
  ret = hal_ioctl (IPIBR_ENABLE_IGMP_SNOOPING, (unsigned long)bridge_name, 0,0,0,0,0);
...
hal_igmp_snooping_disable (char* bridge_name)
{
  ...
  ret = hal_ioctl (IPIBR_DISABLE_IGMP_SNOOPING, (unsigned long)bridge_name,0,0,0,0,0);
...
int
hal_vlan_stacking_disable (u_int32_t ifindex, u_int16_t ethtype,
                           u_int16_t stackmode)
{
  return HAL_SUCCESS;
}
                                          ...
                                          unsigned short svid)
{
  ...
  ret = hal_ioctl (IPIBR_ADD_CVLAN_REG_ENTRY, (unsigned long)name, 
                                          ...
                                          unsigned short svid)
{
  ...
  ret = hal_ioctl (IPIBR_DEL_CVLAN_REG_ENTRY, (unsigned long)name, 
                   ifindex, cvid, svid, 0, 0);
                                   ...
                                   unsigned short trans_vid)
{
  ...
  ret = hal_ioctl (IPIBR_ADD_VLAN_TRANS_ENTRY, (unsigned long)name, 
                                   ...
                                   unsigned short trans_vid)
{
  ...
  ret = hal_ioctl (IPIBR_DEL_VLAN_TRANS_ENTRY, (unsigned long)name, 
                   ifindex, vid, trans_vid, 0, 0);
                                   ...
                                   u_int16_t vid)
{
  ...
  ret = hal_ioctl (IPIBR_SET_PROTO_PROCESS, (unsigned long)bridge_name, 
                            ...
                            unsigned short pvid)
{
  ...
  ret = hal_ioctl (IPI_VLAN_SET_PRO_EDGE_DEFAULT_PVID, (unsigned long)name,
                                    ...
                                    unsigned short untagged_vid)
{
  ...
  ret = hal_ioctl (IPI_VLAN_SET_PRO_EDGE_UNTAGGED_VID, (unsigned long)name,
                            ...
                            unsigned short svid)
{
  ...
  ret = hal_ioctl (IPI_VLAN_ADD_PRO_EDGE_PORT, (unsigned long)name,
                   ifindex, svid, 0, 0, 0);
                            ...
                            unsigned short svid)
{
  ...
  ret = hal_ioctl (IPI_VLAN_DEL_PRO_EDGE_PORT, (unsigned long)name,
                   ifindex, svid, 0, 0, 0);
                             ...
                             u_int16_t vid)
{
  ...
  memset (str, '\0', HAL_BRIDGE_NAME_LEN);
  strncpy (str, bridge_name, HAL_BRIDGE_NAME_LEN);
  memcpy (macaddr, mac, HAL_HW_LENGTH);

  ret = hal_ioctl (IPIBR_GET_IFINDEX_BY_MAC_VID, (unsigned long)str, 
...
int hal_get_bridge_info(char* name, struct bridge_info *info)
{
	...
	ret = hal_ioctl (IPIBR_GET_BRIDGE_INFO, (unsigned long)brname, (unsigned long)info, 0, 0, 0, 0);
	if (ret < 0)
	{
    	return -errno;
	}
...
int hal_get_bridge_port_state(char* name, unsigned int ifindex, unsigned int instance, unsigned int *state)
{
	...
	ret = hal_ioctl (IPIBR_GET_PORT_STATE, (unsigned long)brname, (unsigned long)ifindex, 
					(unsigned long)instance, (unsigned long)state, 0, 0);
	if (ret < 0)
	{
    	return -errno;
	}
