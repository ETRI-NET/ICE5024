...
hal_l2_gmrp_set_service_requirement (char *bridge_name, int vlanid, int ifindex, int serv_req, bool_t activate)
{
 int ret;
  ...
  ret = hal_ioctl (IPIBR_ADD_GMRP_SERVICE_REQ, (unsigned long)bridge_name, vlanid, ifindex, serv_req, activate);
...
hal_gmrp_set_bridge_ext_filter (char *bridge_name, bool_t enable)
{
  ...
  ret = hal_ioctl (IPIBR_SET_EXT_FILTER, (unsigned long)bridge_name, enable, 0, 0, 0);
