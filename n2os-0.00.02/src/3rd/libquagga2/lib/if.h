...
#define CONNECTED_PEER_ORG(C) CHECK_FLAG((C)->flags, ZEBRA_IFA_PEER)

/* Prefix to insert into the RIB */
#define CONNECTED_PREFIX_ORG(C) \
	(CONNECTED_PEER_ORG(C) ? (C)->destination : (C)->address)
...
#define CONNECTED_ID_ORG(C)	\
	((CONNECTED_PEER_ORG(C) && !prefix_match((C)->destination, (C)->address)) ?\
...
#ifndef IFF_NOTRAILERS_ORG
#define IFF_NOTRAILERS_ORG 0x0
#endif /* IFF_NOTRAILERS_ORG */
#ifndef IFF_OACTIVE
