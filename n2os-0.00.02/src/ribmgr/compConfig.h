#if !defined(_ribmgrDluConfig_h)
#define _ribmgrDluConfig_h
...
Int32T  sNosCompName         = RIB_MANAGER;
StringT sNosCompNameStr      = "ribmgr";
StringT sNosCompModuleVer    = "0.0.1";
StringT sNosLogFile          = "ribmgr";
...
void ribInitProcess(void);
void ribTermProcess(void);
void ribRestartProcess(void);
void ribHoldProcess(void);
NosInitProcessT     sNosInitProcess    = ribInitProcess;
NosTermProcessT     sNosTermProcess    = ribTermProcess;
NosRestartProcessT  sNosRestartProcess = ribRestartProcess;
NosHoldProcessT     sNosHoldProcess    = ribHoldProcess;

/* register message(Ipc/Event/Signal) process (SHOULD SET) */
void ribIpcProcess(Int32T msgId, void * data, Uint32T dataLen);
void ribEventProcess(Int32T msgId, void * data, Uint32T dataLen);
void ribSignalProcess(Int32T sig);
NosIpcProcessT    sNosIpcProcess    = ribIpcProcess;
NosEventProcessT  sNosEventProcess  = ribEventProcess;
NosSignalProcessT sNosSignalProcess = ribSignalProcess;

/* register CM/CMSH process (SHOULD SET) */
void ribCmshIpcProcess(Int32T sockId, void *message, Uint32T size);
void ribCmIpcProcess(Int32T sockId, void *message, Uint32T size);
NosCmIpcProcessT sNosCmshIpcProcess = ribCmshIpcProcess;
NosCmIpcProcessT sNosCompIpcProcess = ribCmIpcProcess;
