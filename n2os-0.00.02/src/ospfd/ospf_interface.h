...
#define PRIORITY(I)		((I)->nbr_self->priority)
  ...
  /* Events. */
  void *pTimerHello;               /* timer */
  void *pTimerWait;                /* timer */
  void *pTimerLsAck;              /* timer */
  void *pTimerLsUpdEvent;        /* event */
  void *pTimerLsAckDirect;       /* event */
#ifdef HAVE_OPAQUE_LSA
  void *pTimerOpaqueLsaSelf;     /* Type-9 Opaque-LSAs */
#endif /* HAVE_OPAQUE_LSA */

  /* Updated time */
  struct timeval tvUpdateHello;
...
extern int ospfRouterIdUpdate (nnBufferT *);
extern int ospfInterfaceAdd (nnBufferT *);
extern int ospfInterfaceDelete (nnBufferT *);
extern int ospfInterfaceAddressAdd (nnBufferT *);
extern int ospfInterfaceAddressDelete (nnBufferT *);
extern int ospfInterfaceStateUp (nnBufferT *);
extern int ospfInterfaceStateDown (nnBufferT *);

extern int configWriteOspfInterface(struct cmsh *cmsh);
