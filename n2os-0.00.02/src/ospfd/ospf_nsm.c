...
void
ospfInactivityTimer (Int32T fd, Int16T event, void * arg)
{
  struct ospf_neighbor *nbr;

  nbr = (struct ospf_neighbor *)arg;
  nbr->pTimerInactivity = NULL;
...
void
ospfDbDescTimer (Int32T fd, Int16T event, void * arg)
{
  struct ospf_neighbor *nbr;

  nbr = (struct ospf_neighbor *)arg;
  nbr->pTimerDbDesc = NULL;
  ...
  OSPF_NSM_TIMER_ON (nbr->pTimerDbDesc, ospfDbDescTimer, nbr->v_db_desc);
...
nsm_timer_set (struct ospf_neighbor *nbr)
{
  switch (nbr->state)
    ...
    case NSM_Deleted:
    case NSM_Down:
      OSPF_NSM_TIMER_OFF (nbr->pTimerInactivity);
      OSPF_NSM_TIMER_OFF (nbr->pTimerHelloReply);
    case NSM_Attempt:
    case NSM_Init:
    case NSM_TwoWay:
      OSPF_NSM_TIMER_OFF (nbr->pTimerDbDesc);
      OSPF_NSM_TIMER_OFF (nbr->pTimerLsUpd);
      OSPF_NSM_TIMER_OFF (nbr->pTimerLsReq);
      break;
    case NSM_ExStart:
      OSPF_NSM_TIMER_ON (nbr->pTimerDbDesc, ospfDbDescTimer, nbr->v_db_desc);
      OSPF_NSM_TIMER_OFF (nbr->pTimerLsUpd);
...
nsm_packet_received (struct ospf_neighbor *nbr)
{
  ...
  OSPF_NSM_TIMER_ON (nbr->pTimerInactivity, ospfInactivityTimer,
		     nbr->v_inactivity);
  nbr->tvUpdateInactivity = ospfGetUpdateTime(nbr->v_inactivity, 0);

  if (nbr->oi->type == OSPF_IFTYPE_NBMA && nbr->nbr_nbma)
    OSPF_POLL_TIMER_OFF (nbr->nbr_nbma->pTimerPoll);
...
nsm_notice_state_change (struct ospf_neighbor *nbr, int next_state, int event)
{
  ...
  if (next_state > nbr->state)
    nbr->ts_last_progress = ospfRecentRelativeTime ();
  else /* regression in NSM */
    {
      nbr->ts_last_regress = ospfRecentRelativeTime ();
...
nsm_change_state (struct ospf_neighbor *nbr, int state)
{
  ...
  if ((old_state != NSM_Full && state == NSM_Full) ||
      ...
      if (state == NSM_Full)
	{
	  ...
	  for (x = RIB_ROUTE_TYPE_SYSTEM; x < RIB_ROUTE_TYPE_MAX; x++)
	    {
...
void
ospfNsmEvent (Int32T fd, Int16T ev, void * arg)
{
	int event;
	int next_state;
	struct ospf_neighbor *nbr;
	struct ospfEventArg *oea;

	oea = (struct ospfEventArg *) arg;
	nbr = (struct ospf_neighbor *) oea->arg1;
	event = oea->arg2;
	free(oea);
