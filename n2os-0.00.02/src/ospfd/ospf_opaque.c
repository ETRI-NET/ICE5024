...
{
  OSPF_TIMER_OFF (oi->pTimerOpaqueLsaSelf);
  if (oi->opaque_lsa_self != NULL)
    list_delete (oi->opaque_lsa_self);
...
ospf_opaque_type11_lsa_term (struct ospf *top)
{
  ...
  OSPF_TIMER_OFF (top->pTimerOpaqueLsaSelf);
  if (top->opaque_lsa_self != NULL)
    list_delete (top->opaque_lsa_self);
  ...
  int (* del_lsa_hook)(struct ospf_lsa *lsa))
{
  ...
  new->configWriteRouter = configWriteRouter;
  new->configWriteIf     = configWriteIf;
  new->config_write_debug  = configWriteDebug;
...
free_opaque_info_per_type (void *val)
{
  ...
  OSPF_TIMER_OFF (oipt->pTimerOpaqueLsaSelf);
...
free_opaque_info_per_id (void *val)
{
  ...
  OSPF_TIMER_OFF (oipi->pTimerOpaqueLsaSelf);
  if (oipi->lsa != NULL)
...
static void
opaqueLsaConfigWriteRouterCallback (struct list *funclist,
                                         struct cmsh *cmsh)
{
  struct listnode *node, *nnode;
  struct ospf_opaque_functab *functab;
  ...
  for (ALL_LIST_ELEMENTS (funclist, node, nnode, functab))
    if (functab->configWriteRouter != NULL)
      (* functab->configWriteRouter)(cmsh);
  return;
}

static void
opaqueLsaConfigWriteIfCallback (struct list *funclist,
                                     struct cmsh *cmsh, struct interface *ifp)
{
  struct listnode *node, *nnode;
  struct ospf_opaque_functab *functab;

  for (ALL_LIST_ELEMENTS (funclist, node, nnode, functab))
    if (functab->configWriteIf != NULL)
      (* functab->configWriteIf)(cmsh, ifp);
  return;
}
...
void
ospfOpaqueConfigWriteRouter (struct cmsh *cmsh, struct ospf *ospf)
{
	struct list *funclist;

	if (CHECK_FLAG(ospf->config, OSPF_OPAQUE_CAPABLE))
		cmdPrint(cmsh, " capability opaque");

	funclist = ospf_opaque_wildcard_funclist;
	opaqueLsaConfigWriteRouterCallback(funclist, cmsh);
...
void
ospfOpaqueConfigWriteIf (struct cmsh *cmsh, struct interface *ifp)
{
	struct list *funclist;

	funclist = ospf_opaque_wildcard_funclist;
	opaqueLsaConfigWriteIfCallback(funclist, cmsh, ifp);
...
void
show_opaque_info_detail (struct cmsh *cmsh, struct ospf_lsa *lsa)
{
  ...
  if (cmsh != NULL)
    {
      cmdPrint (cmsh, "  Opaque-Type %u (%s)", opaque_type,
	       ospf_opaque_type_name (opaque_type));
      cmdPrint (cmsh, "  Opaque-ID   0x%x", opaque_id);

      cmdPrint (cmsh, "  Opaque-Info: %u octets of data%s",
  ...
  if ((functab = ospf_opaque_functab_lookup (lsa)) != NULL)
    if (functab->show_opaque_info != NULL)
      (* functab->show_opaque_info)(cmsh, lsa);
  ...
  &&    oi->pTimerOpaqueLsaSelf == NULL)
    {
      ...
      tv.tv_sec = delay;
      tv.tv_usec = 0;
      oi->pTimerOpaqueLsaSelf = taskTimerSet(ospfOpaqueType9LsaOriginate, tv, 0, (void *)oi);
  ...
  &&    area->pTimerOpaqueLsaSelf == NULL)
    {
      ...
      tv.tv_sec = delay;
      tv.tv_usec = 0;
      area->pTimerOpaqueLsaSelf = taskTimerSet(ospfOpaqueType10LsaOriginate, tv, 0, (void *)area);
      delay += OSPF_MIN_LS_INTERVAL;
    }
  ...
  &&    top->pTimerOpaqueLsaSelf == NULL)
    {
      ...
      tv.tv_sec = delay;
      tv.tv_usec = 0;
      top->pTimerOpaqueLsaSelf = taskTimerSet(ospfOpaqueType11LsaOriginate, tv, 0, (void *)top);
      delay += OSPF_MIN_LS_INTERVAL;
    }
  ...
  &&  ! list_isempty (oi->opaque_lsa_self))
    {
	   ...
	   * not being empty.
	   */
          if (oipt->pTimerOpaqueLsaSelf != NULL /* Waiting for a thread call. */
...
void
ospfOpaqueType9LsaOriginate (Int32T fd, Int16T event, void * arg)
{
  struct ospf_interface *oi;
...
void
ospfOpaqueType10LsaOriginate (Int32T fd, Int16T event, void * arg)
{
  struct ospf_area *area;
...
void
ospfOpaqueType11LsaOriginate (Int32T fd, Int16T event, void * arg)
{
  struct ospf *top;
                                      ...
                                      u_char lsa_type, u_char opaque_type)
{
  ...
  if (oipt->pTimerOpaqueLsaSelf != NULL)
    {
      if (IS_DEBUG_OSPF_EVENT)
  ...
  OSPF_OPAQUE_TIMER_ON (oipt->pTimerOpaqueLsaSelf, func, oipt, delay);
...
void
ospfOpaqueType9LsaReoriginateTimer (Int32T fd, Int16T event, void * arg)
{
  struct opaque_info_per_type *oipt;
  struct ospf_opaque_functab *functab;
  struct ospf *top;
  ...
  oipt = (struct opaque_info_per_type *)arg;
  oipt->pTimerOpaqueLsaSelf = NULL;
...
void
ospfOpaqueType10LsaReoriginateTimer (Int32T fd, Int16T event, void * arg)
{
  struct opaque_info_per_type *oipt;
  struct ospf_opaque_functab *functab;
  struct listnode *node, *nnode;
...
void
ospfOpaqueType11LsaReoriginateTimer (Int32T fd, Int16T event, void * arg)
{
  struct opaque_info_per_type *oipt;
  struct ospf_opaque_functab *functab;
  struct ospf *top;
...
ospf_opaque_lsa_refresh_schedule (struct ospf_lsa *lsa0)
{
  ...
  if (oipi->pTimerOpaqueLsaSelf != NULL)
    {
      if (IS_DEBUG_OSPF_EVENT)
  ...
  OSPF_OPAQUE_TIMER_ON (oipi->pTimerOpaqueLsaSelf,
		  ospfOpaqueLsaRefreshTimer, oipi, delay);
out:
  return;
}

void
ospfOpaqueLsaRefreshTimer (Int32T fd, Int16T event, void * arg)
{
  struct opaque_info_per_id *oipi;
  struct ospf_opaque_functab *functab;
  struct ospf_lsa *lsa;
  ...
  oipi = (struct opaque_info_per_id *)arg;
  oipi->pTimerOpaqueLsaSelf = NULL;
