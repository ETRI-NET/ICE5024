...
ospf_apiserver_term (void)
{
   ...
   */
  while ( om->apiserver_list &&
         (apiserv = listgetdata (listhead (om->apiserver_list))) != NULL)
    ospf_apiserver_free (apiserv);

  /* Free client list itself */
  if (om->apiserver_list)
    list_delete (om->apiserver_list);
...
lookup_apiserver (u_char lsa_type, u_char opaque_type)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (om->apiserver_list, n1, apiserv))
    {
...
ospf_apiserver_new (int fd_sync, int fd_async)
{
  ...
  new->pSockFdSyncReadEvent = NULL;
#ifdef USE_ASYNC_READ
  new->pSockFdAsyncReadEvent = NULL;
#endif /* USE_ASYNC_READ */
  new->pSockFdSyncWriteEvent = NULL;
  new->pSockFdAsyncWriteEvent = NULL;
		      ...
		      struct ospf_apiserver *apiserv)
{
  switch (event)
    ...
    case OSPF_APISERVER_ACCEPT:
      taskFdSet(ospfApiserverAccept, fd,
    		  TASK_READ, TASK_PRI_MIDDLE, (void *)apiserv);
      break;
    case OSPF_APISERVER_SYNC_READ:
      apiserv->pSockFdSyncReadEvent =
    		  taskFdSet(ospfApiserverRead, fd,
    				  TASK_READ, TASK_PRI_MIDDLE, (void *)apiserv);
      break;
#ifdef USE_ASYNC_READ
    case OSPF_APISERVER_ASYNC_READ:
      apiserv->pSockFdAsyncReadEvent =
    		  taskFdSet(ospfApiserverRead, fd,
    		      				  TASK_READ, TASK_PRI_MIDDLE, (void *)apiserv);
      break;
#endif /* USE_ASYNC_READ */
	...
	{
	  apiserv->pSockFdSyncWriteEvent =
			  taskFdSet(ospfApiserverSyncWrite, fd,
					  TASK_WRITE, TASK_PRI_MIDDLE, (void *)apiserv);
	}
      break;
    case OSPF_APISERVER_ASYNC_WRITE:
      if (!apiserv->pSockFdAsyncWriteEvent)
	{
	  apiserv->pSockFdAsyncWriteEvent =
			  taskFdSet(ospfApiserverAsyncWrite, fd,
					  TASK_WRITE, TASK_PRI_MIDDLE, (void *)apiserv);
	}
      break;
    }
...
ospf_apiserver_free (struct ospf_apiserver *apiserv)
{
  ...
  if (apiserv->pSockFdSyncReadEvent)
    {
      taskFdDel(apiserv->pSockFdSyncReadEvent);
    }
#ifdef USE_ASYNC_READ
  if (apiserv->pSockFdAsyncReadEvent)
    {
	  taskFdDel (apiserv->pSockFdAsyncReadEvent);
    }
#endif /* USE_ASYNC_READ */
  if (apiserv->pSockFdSyncWriteEvent)
    {
      taskFdDel (apiserv->pSockFdSyncWriteEvent);
    }

  if (apiserv->pSockFdAsyncWriteEvent)
    {
      taskFdDel (apiserv->pSockFdAsyncWriteEvent);
    }
  ...
  listnode_delete (om->apiserver_list, apiserv);

  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("API: Delete apiserv(%p), total#(%d)", apiserv, om->apiserver_list->count);
...
void
ospfApiserverRead (Int32T fd, Int16T event, void *args)
{
  struct ospf_apiserver *apiserv;
  struct msg *msg;

  apiserv = (struct ospf_apiserver *)args;

  if (fd == apiserv->fd_sync)
    {
      event = OSPF_APISERVER_SYNC_READ;
      apiserv->pSockFdSyncReadEvent = NULL;
  ...
  else if (fd == apiserv->fd_async)
    {
      ...
      apiserv->pSockFdAsyncReadEvent = NULL;
...
void
ospfApiserverSyncWrite (Int32T fd, Int16T event, void *args)
{
  struct ospf_apiserver *apiserv;
  struct msg *msg;
  int rc = -1;
  ...
  apiserv->pSockFdSyncWriteEvent = NULL;
...
void
ospfApiserverAsyncWrite (Int32T fd, Int16T event, void *args)
{
  struct ospf_apiserver *apiserv;
  struct msg *msg;
  int rc = -1;
  ...
  apiserv->pSockFdAsyncWriteEvent = NULL;
...
void
ospfApiserverAccept (Int32T fd, Int16T event, void *args)
{
	int accept_sock;
	int new_sync_sock;
	int new_async_sock;
	...
	listnode_add(om->apiserver_list, apiserv);
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("API: New apiserv(%p), total#(%d)", apiserv,
				om->apiserver_list->count);
...
ospf_apiserver_lsa9_originator (void *arg)
{
  ...
  if (listcount (om->apiserver_list) > 0) {
...
void
ospf_apiserver_show_info (struct cmsh *cmsh, struct ospf_lsa *lsa)
{
  struct opaque_lsa
  {
  ...
  if (cmsh != NULL)
    {
	  char showBuff[1024] = {};
      int i, idx = 0;
      cmdPrint (cmsh, "  Added using OSPF API: %u octets of opaque data %s",
	       opaquelen,
	       VALID_OPAQUE_INFO_LEN (lsa->data) ? "" : "(Invalid length?)");
      idx += sprintf(showBuff + idx, "  Opaque data: ");

      for (i = 0; i < opaquelen; i++)
	{
    	  idx += sprintf(showBuff + idx, "0x%x ", olsa->data[i]);
	}
...
ospf_apiserver_clients_notify_all (struct msg *msg)
{
  ...
  for (ALL_LIST_ELEMENTS (om->apiserver_list, node, nnode, apiserv))
    ospf_apiserver_send_msg (apiserv, msg);
}
