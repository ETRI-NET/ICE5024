...
static void ospf_ls_upd_send (struct ospf_neighbor *, struct list *, int);
//static void ospf_ls_upd_send_lsa (struct ospf_neighbor *, struct ospf_lsa *, int);
//static void ospf_ls_req_send (struct ospf_neighbor *);
//static void ospf_db_desc_resend (struct ospf_neighbor *);
//static void ospf_db_desc_send (struct ospf_neighbor *);
static void ospf_packet_add (struct ospf_interface *, struct ospf_packet *);
...

void
ospfLsReqTimer (Int32T fd, Int16T ev, void * arg)
{
  struct ospf_neighbor *nbr;

  nbr = (struct ospf_neighbor *)arg;
  nbr->pTimerLsReq = NULL;
  ...
  OSPF_NSM_TIMER_ON (nbr->pTimerLsReq, ospfLsReqTimer, nbr->v_ls_req);
...
ospf_ls_req_event (struct ospf_neighbor *nbr)
{
  ...
  if (nbr->pTimerLsReq)
    {
      taskTimerDel(nbr->pTimerLsReq);
      nbr->pTimerLsReq = NULL;
    }
  tv.tv_sec = 0; tv.tv_usec = 0;
  nbr->pTimerLsReq = taskTimerSet(ospfLsReqTimer, tv, 0, (void *)nbr);
}

/* Cyclic timer function.  Fist registered in ospf_nbr_new () in
   ospf_neighbor.c  */
void
ospfLsUpdTimer (Int32T fd, Int16T event, void * arg)
{
  struct ospf_neighbor *nbr;

  nbr = (struct ospf_neighbor *)arg;
  nbr->pTimerLsUpd = NULL;
      ...
      for (i = OSPF_MIN_LSA; i < OSPF_MAX_LSA; i++)
	{
	  ...
	  for (rn = route_top (table); rn; rn = route_next (rn))
		...
		/* Don't retransmit an LSA if we received it within
		  ...
		  - MAG */
		if (tv_cmp (tv_sub (ospfRecentRelativeTime (), lsa->tv_recv),
  ...
  OSPF_NSM_TIMER_ON (nbr->pTimerLsUpd, ospfLsUpdTimer, nbr->v_ls_upd);

}

void
ospfLsAckTimer (Int32T fd, Int16T event, void * arg)
{
  ...
  /* Set LS Ack timer. */
  OSPF_ISM_TIMER_ON (oi->pTimerLsAck, ospfLsAckTimer, oi->v_ls_ack);
...
void
ospfWrite (Int32T fd, Int16T event, void *args)
{
  struct ospf *ospf = (struct ospf *)args;
  struct ospf_interface *oi;
  struct ospf_packet *op;
...
#define OSPF_WRITE_IPHL_SHIFT 2
  ...
  {
    ospf->pSockFdWriteEvent =
    		taskFdSet(ospfWrite, ospf->fd,
    				TASK_WRITE, TASK_PRI_MIDDLE, (void *)ospf);
  }
...
#define REJECT_IF_TBIT_ON	1 /* XXX */
  ...
  if (oi->type == OSPF_IFTYPE_NBMA)
	...
	&& IPV4_ADDR_CMP(&BDR(oi), &iph->ip_src))
      OSPF_NSM_TIMER_ON (nbr->pTimerHelloReply, ospfHelloReplyTimer,
...
)
{
  ...
  while (linkbytes)
  {
    thislinklen = OSPF_ROUTER_LSA_LINK_SIZE + 4 * rll->m[0].tos_count;
    ...
    rll = (struct router_lsa_link *)((caddr_t) rll + thislinklen);
...
void
ospfRead (Int32T fd, Int16T event, void *args)
{
  int ret;
  struct stream *ibuf;
  struct ospf *ospf;
...
void
ospfPollTimer (Int32T fd, Int16T event, void * arg)
{
  struct ospf_nbr_nbma *nbr_nbma;

  nbr_nbma = (struct ospf_nbr_nbma *)arg;
  nbr_nbma->pTimerPoll = NULL;
...
void
ospfHelloReplyTimer (Int32T fd, Int16T event, void * arg)
{
  struct ospf_neighbor *nbr;

  nbr = (struct ospf_neighbor *)arg;
  nbr->pTimerHelloReply = NULL;
...
ospf_db_desc_send (struct ospf_neighbor *nbr)
{
  ...
  nbr->last_send_ts = ospfRecentRelativeTime();
}
...
void
ospfLsUpdSendQueueEvent (Int32T fd, Int16T ev, void * arg)
{
  ...
  if (again != 0)
    {
      ...
      oi->pTimerLsUpdEvent = taskTimerSet(ospfLsUpdSendQueueEvent, tv, 0, oi);
    }
...
void
ospfLsAckSendEvent (Int32T fd, Int16T ev, void * arg)
{
  struct ospf_interface *oi = (struct ospf_interface *)arg;

  oi->pTimerLsAckDirect = NULL;
...
ospf_ls_ack_send (struct ospf_neighbor *nbr, struct ospf_lsa *lsa)
{
  ...
  if (oi->pTimerLsAckDirect == NULL)
  {
	  tv.tv_sec = 0; tv.tv_usec = 0;
    oi->pTimerLsAckDirect = taskTimerSet(ospfLsAckSendEvent, tv, 0, (void *)oi);
  }
}
