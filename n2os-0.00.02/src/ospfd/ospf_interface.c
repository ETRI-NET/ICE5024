  ...
  for (ALL_LIST_ELEMENTS (ospf->oiflist, node, nnode, oi))
    if (oi->type != OSPF_IFTYPE_VIRTUALLINK)
        ...
        if (oi->type == OSPF_IFTYPE_POINTOPOINT)
	  {
	    ...
	    if (prefix_match(CONNECTED_PREFIX_ORG(oi->connected),
			     (struct prefix *)&addr))
	      return oi;
	  }
        else
...
ospf_if_lookup_by_prefix (struct ospf *ospf, struct prefix_ipv4 *p)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->oiflist, node, oi))
      ...
      if (oi->type != OSPF_IFTYPE_VIRTUALLINK)
	{
	  ...
	  prefix_copy (&ptmp, CONNECTED_PREFIX_ORG(oi->connected));
	  apply_mask (&ptmp);
...
struct in_addr ribmgrRouterId;

int
ospfRouterIdUpdate (nnBufferT *msgBuff)
{
	struct ospf *ospf;
	PrefixT p;
...
int
ospfInterfaceAdd (nnBufferT *msgBuff)
{
  struct interface *ifp;
...
int
ospfInterfaceDelete (nnBufferT *msgBuff)
{
  struct interface *ifp;
  struct route_node *rn;
...
int
ospfInterfaceAddressAdd (nnBufferT *msgBuff)
{
  struct connected *c;

  c = pifInterfaceAddressRead (EVENT_INTERFACE_ADDRESS_ADD,  msgBuff);
...
int
ospfInterfaceAddressDelete (nnBufferT *msgBuff)
{
  struct connected *c;
  struct interface *ifp;
  struct ospf_interface *oi;
  ...
  c = pifInterfaceAddressRead (EVENT_INTERFACE_ADDRESS_DELETE, msgBuff);
...
interfaceIfLookup (nnBufferT *msgBuff)
{
	...
	return if_lookup_by_name_len(ifName, strnlen(ifName, INTERFACE_NAMSIZ));
}

int
ospfInterfaceStateUp (nnBufferT *msgBuff)
{
  struct interface *ifp;
  struct ospf_interface *oi;
  struct route_node *rn;

  ifp = interfaceIfLookup (msgBuff);
  ...
  if (if_is_operative (ifp))
    {
      ...
      interfaceIfSetValue(msgBuff, ifp);
...
int
ospfInterfaceStateDown (nnBufferT *msgBuff)
{
  struct interface *ifp;
  struct ospf_interface *oi;
...
int
configWriteOspfInterface(struct cmsh *cmsh)
{
		...
		do {
			...
			params->type != OSPF_IFTYPE_LOOPBACK) {
				if (params->type != ospf_default_iftype(ifp)) {
					idx += sprintf(printBuff + idx, " ip ospf network %s",
							ospf_int_type_str[params->type]);
					if (params != IF_DEF_PARAMS(ifp))
						idx += sprintf(printBuff + idx, " %s", inet_ntoa(rn->p.u.prefix4));
			...
			params->auth_type != OSPF_AUTH_NOTSET) {
				...
				idx += sprintf(printBuff + idx, " ip ospf authentication%s", auth_str);
				if (params != IF_DEF_PARAMS(ifp))
					idx += sprintf(printBuff + idx, " %s", inet_ntoa(rn->p.u.prefix4));
				cmdPrint(cmsh, "%s", printBuff);
				idx = 0; memset(printBuff, 0, 1024);
			...
			if (OSPF_IF_PARAM_CONFIGURED(params, auth_simple)
				...
				idx += sprintf(printBuff + idx, " ip ospf authentication-key %s",
						params->auth_simple);
				if (params != IF_DEF_PARAMS(ifp))
					idx += sprintf(printBuff + idx, " %s", inet_ntoa(rn->p.u.prefix4));
				cmdPrint(cmsh, "%s", printBuff);
			...
			for (ALL_LIST_ELEMENTS_RO(params->auth_crypt, n2, ck)) {
				idx += sprintf(printBuff + idx, " ip ospf message-digest-key %d md5 %s",
			...
			if (OSPF_IF_PARAM_CONFIGURED(params, output_cost_cmd)) {
				idx += sprintf(printBuff + idx, " ip ospf cost %u", params->output_cost_cmd);
				if (params != IF_DEF_PARAMS(ifp))
					idx += sprintf(printBuff + idx, " %s", inet_ntoa(rn->p.u.prefix4));
				cmdPrint(cmsh, "%s", printBuff);
				idx = 0; memset(printBuff, 0, 1024);
			...
			params->v_hello != OSPF_HELLO_INTERVAL_DEFAULT) {
				idx += sprintf(printBuff + idx, " ip ospf hello-interval %u", params->v_hello);
				if (params != IF_DEF_PARAMS(ifp))
					idx += sprintf(printBuff + idx, " %s", inet_ntoa(rn->p.u.prefix4));
				cmdPrint(cmsh, "%s", printBuff);
				idx = 0; memset(printBuff, 0, 1024);
			...
			params->v_wait != OSPF_ROUTER_DEAD_INTERVAL_DEFAULT) {
				idx += sprintf(printBuff + idx, " ip ospf dead-interval ");

				/* fast hello ? */
				if (OSPF_IF_PARAM_CONFIGURED(params, fast_hello))
					idx += sprintf(printBuff + idx, "minimal hello-multiplier %d",
							params->fast_hello);
				else
					idx += sprintf(printBuff + idx, "%u", params->v_wait);
			...
			params->priority != OSPF_ROUTER_PRIORITY_DEFAULT) {
				idx += sprintf(printBuff + idx, " ip ospf priority %u", params->priority);
				if (params != IF_DEF_PARAMS(ifp))
					idx += sprintf(printBuff + idx, " %s", inet_ntoa(rn->p.u.prefix4));
				cmdPrint(cmsh, "%s", printBuff);
				idx = 0; memset(printBuff, 0, 1024);
			...
			params->retransmit_interval != OSPF_RETRANSMIT_INTERVAL_DEFAULT) {
				idx += sprintf(printBuff + idx, " ip ospf retransmit-interval %u",
			...
			params->transmit_delay != OSPF_TRANSMIT_DELAY_DEFAULT) {
				idx += sprintf(printBuff + idx, " ip ospf transmit-delay %u",
			...
			params->mtu_ignore != OSPF_MTU_IGNORE_DEFAULT) {
				if (params->mtu_ignore == 0)
					idx += sprintf(printBuff + idx, " no ip ospf mtu-ignore");
				else
					idx += sprintf(printBuff + idx, " ip ospf mtu-ignore");
				if (params != IF_DEF_PARAMS(ifp))
					idx += sprintf(printBuff + idx, " %s", inet_ntoa(rn->p.u.prefix4));
				cmdPrint(cmsh, "%s", printBuff);
				idx = 0; memset(printBuff, 0, 1024);
...
#ifdef HAVE_OPAQUE_LSA
		ospfOpaqueConfigWriteIf(cmsh, ifp);
#endif /* HAVE_OPAQUE_LSA */
	}
