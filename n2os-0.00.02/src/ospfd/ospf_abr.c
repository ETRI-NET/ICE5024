...
ospf_abr_should_accept (struct prefix_ipv4 *p, struct ospf_area *area)
{
  if (IMPORT_NAME (area))
	...
	IMPORT_LIST (area) = access_list_lookup (AFI_IP, IMPORT_NAME (area));
      ...
      if (IMPORT_LIST (area))
        if (access_list_apply (IMPORT_LIST (area), p) == FILTER_DENY_ORG)
           return 0;
    }
			 ...
			 struct prefix_ipv4 *p)
{
  if (PREFIX_NAME_IN (area))
	...
	if (prefix_list_apply (PREFIX_LIST_IN (area), p) != PREFIX_PERMIT_ORG)
	  return 0;
    }
  return 1;
}
			  ...
			  struct prefix_ipv4 *p)
{
  if (PREFIX_NAME_OUT (area))
	...
	if (prefix_list_apply (PREFIX_LIST_OUT (area), p) != PREFIX_PERMIT_ORG)
	  return 0;
    }
  return 1;
}
			  ...
			  struct prefix_ipv4 *p, struct ospf_route *or)
{
  ...
  if (EXPORT_NAME (area))
	...
	EXPORT_LIST (area) = access_list_lookup (AFI_IP, EXPORT_NAME (area));
      ...
      if (EXPORT_LIST (area))
        if (access_list_apply (EXPORT_LIST (area), p) == FILTER_DENY_ORG)
           return 0;
    }
...
void
ospfAbrTaskTimer (Int32T fd, Int16T event, void * arg)
{
...
ospf_schedule_abr_task (struct ospf *ospf)
{
  ...
  if (ospf->pTimerAbrTask == NULL)
  {
	struct timeval tv = {0,0};
	tv.tv_sec = OSPF_ABR_TASK_DELAY;

    ospf->pTimerAbrTask =
    		taskTimerSet(ospfAbrTaskTimer, tv, 0, (void *)ospf);
  }
}
