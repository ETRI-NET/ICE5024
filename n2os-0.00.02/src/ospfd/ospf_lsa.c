...
get_age (struct ospf_lsa *lsa)
{
  ...
  age = ntohs (lsa->data->ls_age) 
        + tv_floor (tv_sub (ospfRecentRelativeTime (), lsa->tv_recv));
...
ospf_lsa_new ()
{
  ...
  new->tv_recv = ospfRecentRelativeTime ();
...
lsa_link_ptop_set (struct stream *s, struct ospf_interface *oi)
{
  ...
  id.s_addr = CONNECTED_PREFIX_ORG(oi->connected)->u.prefix4.s_addr & mask.s_addr;
...

void
ospfStubRouterTimer (Int32T fd, Int16T event, void * arg)
{
  struct ospf_area *area = (struct ospf_area *)arg;
  
  area->pTimerStubRouter = NULL;
...
ospf_stub_router_check (struct ospf_area *area)
{
  ...
  OSPF_AREA_TIMER_ON (area->pTimerStubRouter, ospfStubRouterTimer,
                      area->ospf->stub_router_startup_time);
  area->tvUpdateStubRouter
  	  	  = ospfGetUpdateTime(area->ospf->stub_router_startup_time, 0);
}
 
...

static void
stream_put_ospf_metric (struct stream *s, u_int32_t metricVal)
{
  u_int32_t metric;
  char *mp;

  /* Put 0 metric. TOS metric is not supported. */
  metric = htonl (metricVal);
  mp = (char *) &metric;
  mp++;
  stream_put (s, mp, 3);
}
...
void
ospfExternalLsaOriginateTimer (Int32T fd, Int16T ev, void * arg)
{
  struct ospf *ospf = (struct ospf *)arg;
  struct route_node *rn;
  struct external_info *ei;
  struct route_table *rt;
  int type = ospf->routerType;

  ospf->pTimerExternalLsa = NULL;
...
void
ospfDefaultOriginateTimer (Int32T fd, Int16T event, void * arg)
{
  struct prefix_ipv4 p;
...
void
ospfMaxageLsaRemover (Int32T fd, Int16T event, void * arg)
{
  struct ospf *ospf = (struct ospf *)arg;
  struct ospf_lsa *lsa;
  struct route_node *rn;
	...
	if ((lsa = rn->info) == NULL)
	  {
        ...
        OSPF_TIMER_ON (ospf->pTimerMaxage, ospfMaxageLsaRemover, 0);
  ...
  if (reschedule)
    OSPF_TIMER_ON (ospf->pTimerMaxage, ospfMaxageLsaRemover,
                   ospf->maxage_delay);
...
void
ospfLsaMaxageWalker (Int32T fd, Int16T event, void * arg)
{
  struct ospf *ospf = (struct ospf *)arg;
  struct route_node *rn;
  struct ospf_lsa *lsa;
  ...
  OSPF_TIMER_ON (ospf->pTimerMaxageWalker, ospfLsaMaxageWalker,
...
ospf_flush_self_originated_lsas_now (struct ospf *ospf)
{
   ...
   */
  if (ospf->pTimerMaxage != NULL)
    {
      OSPF_TIMER_OFF (ospf->pTimerMaxage);
      ospfMaxageLsaRemover(0, 0, ospf);
    }
...
static void
ospfLsaAction (Int32T fd, Int16T ev, void * arg)
{
  struct lsa_action *data;

  data = (struct lsa_action *)arg;
...
ospf_schedule_lsa_flood_area (struct ospf_area *area, struct ospf_lsa *lsa)
{
  ...
  taskTimerSet(ospfLsaAction, tv, 0, (void *)data);
}
...
ospf_refresher_register_lsa (struct ospf *ospf, struct ospf_lsa *lsa)
{
  ...
  if (lsa->refresh_list < 0)
	...
	delay = 0;
      ...
      current_index = ospf->lsa_refresh_queue.index + (time (NULL)
                - ospf->lsa_refresher_started)/OSPF_LSA_REFRESHER_GRANULARITY;
      
      idx = (current_index + delay/OSPF_LSA_REFRESHER_GRANULARITY)
	      % (OSPF_LSA_REFRESHER_SLOTS);

      if (IS_DEBUG_OSPF (lsa, LSA_REFRESH))
	zlog_debug ("LSA[Refresh]: lsa %s with age %d added to index %d",
		   inet_ntoa (lsa->data->id), LS_AGE (lsa), idx);
      if (!ospf->lsa_refresh_queue.qs[idx])
	ospf->lsa_refresh_queue.qs[idx] = list_new ();
      listnode_add (ospf->lsa_refresh_queue.qs[idx],
                    ospf_lsa_lock (lsa)); /* lsa_refresh_queue */
      lsa->refresh_list = idx;
      if (IS_DEBUG_OSPF (lsa, LSA_REFRESH))
        zlog_debug ("LSA[Refresh:%s]: ospf_refresher_register_lsa(): "
                   "setting refresh_list on lsa %p (slod %d)", 
                   inet_ntoa (lsa->data->id), lsa, idx);
    }
}
...
void
ospfLsaRefreshWalker (Int32T fd, Int16T event, void * arg)
{
  ...
  tv.tv_usec = 0;
  ospf->pTimerLsaRefresher = taskTimerSet(ospfLsaRefreshWalker, tv, 0, (void *)ospf);
