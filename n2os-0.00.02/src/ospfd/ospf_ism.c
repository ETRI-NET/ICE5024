...

void
ospfHelloTimer (Int32T fd, Int16T event, void * arg)
{
...
void
ospfWaitTimer (Int32T fd, Int16T event, void * arg)
{
  struct ospf_interface *oi;
  ...
  switch (oi->state)
    ...
    case ISM_Down:
	 ...
	 reset also. */
      if(oi->pTimerHello)
      {
    	  taskTimerDel(oi->pTimerHello);
    	  oi->pTimerHello = NULL;
      }
      if(oi->pTimerWait)
      {
    	  taskTimerDel(oi->pTimerWait);
    	  oi->pTimerWait = NULL;
      }
      if(oi->pTimerLsAck)
		{
		  taskTimerDel(oi->pTimerLsAck);
		  oi->pTimerLsAck = NULL;
		}
      break;
    case ISM_Loopback:
    ...
    case ISM_Waiting:
	 ...
	 BDRouter. The router begin to receive and send Hello Packets. */
      ...
      OSPF_ISM_TIMER_MSEC_ON (oi->pTimerHello, ospfHelloTimer, 1);
      oi->tvUpdateHello = ospfGetUpdateTime(0, 1);
      OSPF_ISM_TIMER_ON (oi->pTimerWait, ospfWaitTimer,
...
void
ospfIsmEvent (Int32T fd, Int16T ev, void * arg)
{
  int event;
  int next_state;
  struct ospf_interface *oi;
  ...
  oi = (struct ospf_interface *)oea->arg1;
  event = oea->arg2;
  free(oea);
