...
{
  return (route_type == RIB_ROUTE_TYPE_MAX) ?
...
const char *
ospf_timer_dump (void *ev, struct timeval uptime, char *buf, size_t size)
{
  ...
  result = tv_sub (uptime, ospfRecentRelativeTime());
...
int
config_write_debug (struct cmsh *cmsh)
{
	int i, r;
	...
	if (r == (OSPF_DEBUG_SEND_RECV | OSPF_DEBUG_DETAIL)) {
		cmdPrint(cmsh, "debug ospf packet all detail");
		return 1;
	}
	...
	if (r == OSPF_DEBUG_SEND_RECV) {
		cmdPrint(cmsh, "debug ospf packet all");
		for (i = 0; i < 5; i++)
			if (conf_debug_ospf_packet[i] & OSPF_DEBUG_DETAIL)
				cmdPrint(cmsh, "debug ospf packet %s detail", type_str[i]);
		return 1;
	}
	...
	for (i = 0; i < 5; i++) {
		...
		cmdPrint(cmsh, "debug ospf packet %s%s", type_str[i],
