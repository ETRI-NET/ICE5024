...
#define EXTERNAL_INFO(T)      om->external_info[T]
	...
	/* OSPF Routemap related pointers */
	...
	/* OSPF redistribute information. */
	Uint8T ospfRedistributeDefault;
	Uint8T ospfRedistribute[RIB_ROUTE_TYPE_MAX];
	Uint8T ospfDefaultInformation;
...
struct ospf
{
	int routerType;				/* Zebra route's types. */
...
#define DEFAULT_ORIGINATE_ALWAYS	2
  ...
  /* Events. */
  void *pTimerAbrTask;            /* ABR task timer. */
  void *pTimerAsbrCheck;          /* ASBR check timer. */
  void *pTimerDistributeUpdate;   /* Distirbute list update timer. */
  void *pTimerAseCalc;		/* ASE calculation timer. */
  void *pTimerSpfCalc;	        /* SPF calculation timer. */
  void *pTimerExternalLsa;	/* AS-external-LSA origin timer. */
#ifdef HAVE_OPAQUE_LSA
  void *pTimerOpaqueLsaSelf;	/* Type-11 Opaque-LSAs origin event. */
#endif /* HAVE_OPAQUE_LSA */
  void *pTimerMaxageWalker;       /* MaxAge LSA checking timer. */

  /* Updated time. */
  struct timeval tvUpdateSpfCalc;


#define OSFP_LSA_MAXAGE_REMOVE_DELAY_DEFAULT	60
  unsigned int maxage_delay;		/* Delay on Maxage remover timer, sec */
  void *pTimerMaxage;              /* MaxAge LSA remover timer. */
#define OSPF_LSA_MAXAGE_CHECK_INTERVAL		30

  void *pTimerDeferredShutdown;	/* deferred/stub-router shutdown timer*/
  struct timeval tvUpdateDS;

  void *pSockFdWriteEvent;
  void *pSockFdReadEvent;
  int fd;
...
#define DISTRIBUTE_LIST(O,T)    (O)->dlist[T].list
  ...
  } route_map [RIB_ROUTE_TYPE_MAX + 1]; /* +1 is for default-information */
...
#define PREFIX_NAME_OUT(A)  (A)->plist_out.name
  ...
  void *pTimerStubRouter;    /* Stub-router timer */
  struct timeval tvUpdateStubRouter;

  /* Events. */
#ifdef HAVE_OPAQUE_LSA
  void *pTimerOpaqueLsaSelf;	/* Type-10 Opaque-LSAs origin. */
#endif /* HAVE_OPAQUE_LSA */
...
#define OSPF_POLL_TIMER_ON(T,F,V)                                             \
    do {                                                                      \
	  ...
	  tv.tv_usec = 0;																	\
	  if (!(T))                                                               \
	  	  (T) = taskTimerSet((F), tv, 0, nbr_nbma);  					                    \
    } while (0)
...
extern Int32T configWriteOspf(struct cmsh *);
extern void ospfVersionUpdate (void);
