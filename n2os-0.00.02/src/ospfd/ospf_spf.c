                    ...
                    struct route_table *new_rtrs)
{
  ...
  *(v->stat) = LSA_SPF_IN_SPFTREE;
  ...
  area->ospf->ts_spf = ospfRecentRelativeTime();
...
void
ospfSpfCalculateTimer (Int32T fd, Int16T event, void * arg)
{
...
ospf_spf_calculate_schedule (struct ospf *ospf)
{
  ...
  ospf->pTimerSpfCalc = taskTimerSet(ospfSpfCalculateTimer, tv, 0, (void *)ospf);
  ospf->tvUpdateSpfCalc = ospfGetUpdateTime(0, delay);
}
