...
void
ospf_intra_add_stub (struct route_table *rt, struct router_lsa_link *rll,
		     ...
		     int parent_is_root, int lsa_pos)
{
  ...
  p.family = AF_INET;
  p.prefix = rll->link_id;
  p.prefixlen = ip_masklen (rll->link_data);
  apply_mask_ipv4 (&p);
  ...
  /* (1) Calculate the distance D of stub network from the root.  D is
     ...
     cost. */
  cost = v->distance + ntohs (rll->m[0].metric);

  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("ospf_intra_add_stub(): calculated cost is %d + %d = %d", 
	       v->distance, ntohs(rll->m[0].metric), cost);
   ...
   */
  if (parent_is_root && rll->link_data.s_addr == 0xffffffff &&
      ospf_if_lookup_by_local_addr (area->ospf, NULL, rll->link_id))
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: ignoring host route %s/32 to self.",
                    __func__, inet_ntoa (rll->link_id));
      return;
    }
