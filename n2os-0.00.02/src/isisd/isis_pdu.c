...
process_p2p_hello (struct isis_circuit *circuit)
{
  ...
  tv.tv_sec = (long)adj->hold_time;
  adj->pTimerExpire = taskTimerSet(isisAdjExpire, tv, 0, (void *)adj);
...
process_lan_hello (int level, struct isis_circuit *circuit, u_char * ssnpa)
{
	...
	if (isis->debugs & DEBUG_ADJ_PACKETS) {
		zlog_debug("ISIS-Adj (%s): Rcvd L%d LAN IIH on %s, cirType %s, "
				"cirID %u", circuit->area->area_tag, level,
				circuit->interface->name, circuit_t2string(circuit->is_type),
				circuit->circuit_id);
		if (isis->debugs & DEBUG_PACKET_DUMP)
			zlog_dump_data(STREAM_DATA(circuit->rcv_stream),
					stream_get_endp(circuit->rcv_stream));
	...
	if (hdr.pdu_len < (ISIS_FIXED_HDR_LEN + ISIS_LANHELLO_HDRLEN)
		...
		zlog_warn("ISIS-Adj (%s): Rcvd LAN IIH from (%s) with "
				"invalid pdu length %d", circuit->area->area_tag,
				circuit->interface->name, hdr.pdu_len);
		return ISIS_WARNING;
	}
	...
	retval = parse_tlvs(circuit->area->area_tag,
			STREAM_PNT(circuit->rcv_stream),
			hdr.pdu_len - ISIS_LANHELLO_HDRLEN - ISIS_FIXED_HDR_LEN, &expected,
			&found, &tlvs, &auth_tlv_offset);
	...
	if (circuit->passwd.type) {
		if (!(found & TLVFLAG_AUTH_INFO)
				|| authentication_check(&tlvs.auth_info, &circuit->passwd,
						circuit->rcv_stream, auth_tlv_offset)) {
			isis_event_auth_failure(circuit->area->area_tag,
					"LAN hello authentication failure", hdr.source_id);
			retval = ISIS_WARNING;
			goto out;
		}
	}
	...
	out: if (isis->debugs & DEBUG_ADJ_PACKETS) {
		zlog_debug("ISIS-Adj (%s): Rcvd L%d LAN IIH from %s on %s, cirType %s, "
				"cirID %u, length %ld", circuit->area->area_tag, level,
				snpa_print(ssnpa), circuit->interface->name,
				circuit_t2string(circuit->is_type), circuit->circuit_id,
				(long int)stream_get_endp(circuit->rcv_stream));
	}
...
void
isisReceive (Int32T fd, Int16T event, void *args)
{
...
void
isisReceive (Int32T fd, Int16T event, void * arg)
{
  struct isis_circuit *circuit;
  u_char ssnpa[ETH_ALEN];
  int retval;
  ...
  if (!circuit->is_passive)
  {
	  tv.tv_sec = (circuit->area->circuit_list) / 10;
	  tv.tv_usec = 1000 * ((circuit->area->circuit_list) % 10);
	  circuit->pSockFdEvent = taskTimerSet (isisReceive, tv, 0, (void *)circuit);
  }
...
void
sendLanL1Hello (Int32T fd, Int16T event, void * arg)
{
  struct isis_circuit *circuit;
  struct timeval tv = {0,0};
...
void
sendLanL2Hello (Int32T fd, Int16T event, void * arg)
{
  struct isis_circuit *circuit;
  struct timeval tv = {0,0};
...
void
sendP2pHello (Int32T fd, Int16T event, void * arg)
{
  struct isis_circuit *circuit;
  struct timeval tv = {0,0};
  ...
  /* set next timer event */
  tv.tv_sec = circuit->hello_interval[1];
  circuit->u.p2p.pTimerSendP2pHello = taskTimerSet(sendP2pHello, tv, 0, (void *)circuit);
}
      ...
      if (isis->debugs & DEBUG_SNP_PACKETS)
        {
          zlog_debug ("ISIS-Snp (%s): Sent L%d CSNP on %s, length %ld",
                      circuit->area->area_tag, level, circuit->interface->name,
                      (long int)stream_get_endp (circuit->snd_stream));
...
void
sendL1Csnp (Int32T fd, Int16T event, void * arg)
{
  struct isis_circuit *circuit;
  struct timeval tv = {0,0};
...
void
sendL2Csnp (Int32T fd, Int16T event, void * arg)
{
  struct isis_circuit *circuit;
  struct timeval tv = {0,0};
...
send_psnp (int level, struct isis_circuit *circuit)
{
  ...
  if (circuit->area == NULL || circuit->area->lspdb[level - 1] == NULL ||
		  	  	  	  	  	  dict_count (circuit->area->lspdb[level - 1]) == 0)
    return ISIS_OK;
...
void
sendL1Psnp (Int32T fd, Int16T event, void * arg)
{
 ...
 */
void
sendL2Psnp (Int32T fd, Int16T event, void * arg)
{
  struct isis_circuit *circuit;
  struct timeval tv = {0,0};
...
void
sendLsp (Int32T fd, Int16T event, void * arg)
{
  struct isis_circuit *circuit;
  struct isis_lsp *lsp;
  struct listnode *node;
  ...
  if (circuit->tx (circuit, lsp->level) != ISIS_OK)
    {
