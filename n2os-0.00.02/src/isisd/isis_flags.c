...
flags_get_index (struct flags *flags)
{
  ...
  if (flags->free_idcs == NULL || flags->free_idcs->count == 0)
  ...
  else
    {
      ...
      idx = (long int) listgetdata (node);
      listnode_delete (flags->free_idcs, (void *) idx);
      idx--;
    }
...
void
flags_free_index (struct flags *flags, long int idx)
{
  if (idx + 1 == flags->maxindex)
    {
      flags->maxindex--;
      return;
    }
  ...
  listnode_add (flags->free_idcs, (void *) (idx + 1));
