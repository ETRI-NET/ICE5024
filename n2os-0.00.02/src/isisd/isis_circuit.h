...
struct isis_bcast_info
{
  ...
  void *pTimerRunDr[2]; /* DR election timer */

  void *pTimerSendLanHello[2];	/* send LAN IIHs in this timer */
  ...
  void *pTimerRefreshPseudoLsp[2]; /* refresh pseudo-node LSPs */
};

struct isis_p2p_info
{
  struct isis_adjacency *neighbor;
  void *pTimerSendP2pHello;  /* send P2P IIHs in this timer  */
};
...
struct isis_circuit
{
   ...
   * Events
   */
  void *pTimerSendCsnp[2];
  void *pTimerSendPsnp[2];
  void *pSockFdEvent;
...
void isisCircuitPrintCmsh (struct isis_circuit *circuit, struct cmsh *cmsh,
                             char detail);


Int32T configWriteIsisInterface(struct cmsh *);
