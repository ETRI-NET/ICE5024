...
isis_spftree_del (struct isis_spftree *spftree)
{
	...
	{
		taskTimerDel(spftree->pTimerSpf);
		spftree->pTimerSpf = NULL;
	}
...
void
isisRunSpfL1 (Int32T fd, Int16T event, void * arg)
{
  ...
  if (area->ip_circuits)
    isis_run_spf (area, 1, AF_INET, isis->sysid);

}

void
isisRunSpfL2 (Int32T fd, Int16T event, void * arg)
{
  struct isis_area *area;
  ...
  if (area->ip_circuits)
    isis_run_spf (area, 2, AF_INET, isis->sysid);
...
isis_spf_schedule (struct isis_area *area, int level)
{
  ...
  if (level == 1)
  {
	  tv.tv_sec = area->min_spf_interval[0] - diff;
      spftree->pTimerSpf = taskTimerSet(isisRunSpfL1, tv, 0, (void *)area);
  }
  else
  {
	  tv.tv_sec = area->min_spf_interval[1] - diff;
	  spftree->pTimerSpf = taskTimerSet(isisRunSpfL2, tv, 0, (void *)area);
  }
...
#ifdef HAVE_IPV6
static void
isisRunSpf6L1 (Int32T fd, Int16T event, void * arg)
{
  struct isis_area *area;
  ...
  area->spftree6[0]->pTimerSpf = NULL;
  ...
  if (area->ipv6_circuits)
    isis_run_spf (area, 1, AF_INET6, isis->sysid);

}

static void
isisRunSpf6L2 (Int32T fd, Int16T event, void * arg)
{
  struct isis_area *area;
  ...
  area->spftree6[1]->pTimerSpf = NULL;
  ...
  if (area->ipv6_circuits)
    isis_run_spf (area, 2, AF_INET6, isis->sysid);
...
isis_spf_schedule6 (struct isis_area *area, int level)
{
  ...
  if (level == 1)
  {
	  tv.tv_sec = area->min_spf_interval[0] - diff;
	  tv.tv_usec = 0;
	  spftree->pTimerSpf = taskTimerSet(isisRunSpf6L1, tv, 0, area);
  }
  else
  {
	  tv.tv_sec = area->min_spf_interval[1] - diff;
	  tv.tv_usec = 0;
	  spftree->pTimerSpf = taskTimerSet(isisRunSpf6L2, tv, 0, area);
  }
...
void
isisPrintPaths (struct cmsh *cmsh, struct list *paths, u_char *root_sysid)
{
	struct listnode *node;
	...
	for (ALL_LIST_ELEMENTS_RO(paths, node, vertex)) {
		if (memcmp(vertex->N.id, root_sysid, ISIS_SYS_ID_LEN) == 0) {
			idx += sprintf(printBuff + idx, "%-20s %-12s %-6s%-30s",
					print_sys_hostname(root_sysid), "", "", "");
		} else {
			int rows = 0;
			idx += sprintf(printBuff + idx, "%-20s %-12s %-6u ",
					vid2string(vertex, buff), vtype2string(vertex->type),
					vertex->d_N);
			for (ALL_LIST_ELEMENTS_RO(vertex->Adj_N, anode, adj)) {
				if (adj) {
					if (rows) {
						...
						idx += sprintf(printBuff + idx, "%-20s %-12s %-6s ",
								"", "", "");
					}
					idx += sprintf(printBuff + idx, "%-20s %-9s ",
		...
		if (listcount (vertex->parents) > 0) {
			...
			for (ALL_LIST_ELEMENTS_RO(vertex->parents, pnode, pvertex)) {
				if (rows) {
					cmdPrint(cmsh, "%s",printBuff);
					idx = 0; memset(printBuff, 0, 1024);
					idx += sprintf(printBuff + idx, "%-72s", "");
				}
				idx += sprintf(printBuff + idx, "%s(%d)",
									vid2string(pvertex, buff),
									pvertex->type);
				++rows;
			}
		} else {
			idx += sprintf(printBuff + idx, "  NULL ");
		}
