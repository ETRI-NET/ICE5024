...
time2string (u_int32_t t)
{
  ...
  if (t / SECS_PER_YEAR)
    pos += sprintf (pos, "%uY", t / SECS_PER_YEAR);
  rest = t % SECS_PER_YEAR;
...
print_sys_hostname (u_char *id)
{
  ...
  if (memcmp(id, isis->sysid, ISIS_SYS_ID_LEN) == 0)
