...
void
lspPrint (struct isis_lsp *lsp, struct cmsh *cmsh, char dynhost)
{
	u_char LSPid[255];
	char age_out[8];
	...
	idx += sprintf(printBuff + idx, "%-21s%c  %5u   0x%08x  0x%04x  ",
						LSPid,
						lsp->own_lsp ? '*' : ' ',
						ntohs(lsp->lsp_header->pdu_len),
						ntohl(lsp->lsp_header->seq_num),
	...
	if (ntohs(lsp->lsp_header->rem_lifetime) == 0) {
		...
		age_out[7] = '\0';
		idx += sprintf(printBuff + idx, "%7s   ", age_out);
	} else {
		idx += sprintf(printBuff + idx, " %5u    ",
				ntohs(lsp->lsp_header->rem_lifetime));
	}
	idx += sprintf(printBuff + idx, "%s",
...
void
lspPrintDetail (struct isis_lsp *lsp, struct cmsh *cmsh, char dynhost)
{
	...
	if (lsp->tlv_data.nlpids) {
		for (i = 0; i < lsp->tlv_data.nlpids->count; i++) {
			...
			case NLPID_IP:
			case NLPID_IPV6:
				cmdPrint(cmsh, "  NLPID       : 0x%X",
						lsp->tlv_data.nlpids->nlpids[i]);
				break;
			default:
				cmdPrint(cmsh, "  NLPID       : %s\n", "unknown");
				break;
			}
		}
	}
	...
	if (lsp->tlv_data.hostname) {
		...
		cmdPrint(cmsh, "  Hostname    : %s", hostname);
	}
	...
	if (lsp->tlv_data.auth_info.type != ISIS_PASSWD_TYPE_UNUSED) {
		...
		{
			cmdPrint(cmsh, "  Auth type   : md5");
		}
		else if (lsp->tlv_data.auth_info.type == ISIS_PASSWD_TYPE_CLEARTXT)
		{
			cmdPrint(cmsh, "  Auth type   : clear text");
		}
	}
	...
	if (lsp->tlv_data.router_id) {
		memcpy(ipv4_address, inet_ntoa(lsp->tlv_data.router_id->id),
				sizeof(ipv4_address));
		cmdPrint(cmsh, "  Router ID   : %s", ipv4_address);
	}
	...
	if (lsp->tlv_data.ipv4_addrs)
		for (ALL_LIST_ELEMENTS_RO(lsp->tlv_data.ipv4_addrs, lnode, ipv4_addr)) {
			...
			cmdPrint(cmsh, "  IPv4 Address: %s", ipv4_address);
		}
	...
	if (lsp->tlv_data.ipv4_int_reachs)
		for (ALL_LIST_ELEMENTS_RO(lsp->tlv_data.ipv4_int_reachs, lnode,
			...
			cmdPrint(cmsh, "  Metric      : %-8d IPv4-Internal : %s %s",
	...
	if (lsp->tlv_data.ipv4_ext_reachs)
		for (ALL_LIST_ELEMENTS_RO(lsp->tlv_data.ipv4_ext_reachs, lnode,
			...
			memcpy(ipv4_reach_mask, inet_ntoa(ipv4_reach->mask),
					sizeof(ipv4_reach_mask));
			cmdPrint(cmsh, "  Metric      : %-8d IPv4-External : %s %s",
					ipv4_reach->metrics.metric_default, ipv4_reach_prefix,
					ipv4_reach_mask);
		}
	...
	if (lsp->tlv_data.ipv6_reachs)
		for (ALL_LIST_ELEMENTS_RO(lsp->tlv_data.ipv6_reachs, lnode, ipv6_reach)
			...
			CTRL_INFO_DISTRIBUTION) == DISTRIBUTION_INTERNAL)
				cmdPrint(cmsh, "  Metric      : %-8d IPv6-Internal : %s/%d\n",
						ntohl(ipv6_reach->metric), buff, ipv6_reach->prefix_len);
			else
				cmdPrint(cmsh, "  Metric      : %-8d IPv6-External : %s/%d\n",
						ntohl(ipv6_reach->metric), buff, ipv6_reach->prefix_len);
		}
#endif
	...
	if (lsp->tlv_data.te_is_neighs)
		for (ALL_LIST_ELEMENTS_RO(lsp->tlv_data.te_is_neighs, lnode,
				te_is_neigh)) {
			lspid_print(te_is_neigh->neigh_id, LSPid, dynhost, 0);
			cmdPrint(cmsh, "  Metric      : %-8d IS-Extended   : %s",
					GET_TE_METRIC(te_is_neigh), LSPid);
		}
	...
	if (lsp->tlv_data.te_ipv4_reachs)
		for (ALL_LIST_ELEMENTS_RO(lsp->tlv_data.te_ipv4_reachs, lnode,
			...
			cmdPrint(cmsh, "  Metric      : %-8d IPv4-Extended : %s/%d",
...
Int32T
lspPrintAll (struct cmsh *cmsh, dict_t * lspdb, char detail, char dynhost)
{
	...
	if (detail == ISIS_UI_LEVEL_BRIEF) {
		while (node != NULL) {
			...
			lspPrint(dnode_get(node), cmsh, dynhost);
			node = next;
			lsp_count++;
		}
		...
		while (node != NULL) {
			next = dict_next(lspdb, node);
			lspPrintDetail(dnode_get(node), cmsh, dynhost);
			node = next;
			lsp_count++;
		}
	}
...
lsp_refresh_time (struct isis_lsp *lsp, u_int16_t rem_lifetime)
{
  ...
  refresh_time = area->lsp_refresh[level - 1];
	...
	case CIRCUIT_T_P2P:
	  ...
	  if (nei && (level & nei->circuit_t))
	      ...
	      if (area->newmetric)
		{
		  ...
		  metric1 = circuit->te_metric[level - 1];
		  SET_TE_METRIC(te_is_neigh, metric1);
		  listnode_add (tlv_data.te_is_neighs, te_is_neigh);
		}
	    }
	  break;
...
lsp_generate (struct isis_area *area, int level)
{
  ...
  if (level == IS_LEVEL_1)
  {
	  area->pTimerLspRefresh[level - 1] = taskTimerSet(lspL1Refresh, tv, 0, (void *)area);
  }
  else if (level == IS_LEVEL_2)
  {
	  area->pTimerLspRefresh[level -1] = taskTimerSet(lspL2Refresh, tv, 0, (void *)area);
  }
 ...
 */
void
lspL1Refresh (Int32T fd, Int16T event, void *args)
{
  struct isis_area *area;

  area = (struct isis_area *)args;
  ...
  lsp_regenerate (area, IS_LEVEL_1);
}

void
lspL2Refresh (Int32T fd, Int16T event, void *args)
{
  struct isis_area *area;
  ...
  lsp_regenerate (area, IS_LEVEL_2);
}
  ...
  for (lvl = IS_LEVEL_1; lvl <= IS_LEVEL_2; lvl++)
    {
       ...
       * Throttle avoidance
       */
      if(area->pTimerLspRefresh[lvl - 1])
      {
    	  taskTimerDel(area->pTimerLspRefresh[lvl - 1]);
    	  area->pTimerLspRefresh[lvl - 1] = NULL;
      }
      ...
      if (diff < area->lsp_gen_interval[lvl - 1])
        {
          ...
          if (lvl == IS_LEVEL_1)
          {
        	  tv.tv_sec = area->lsp_gen_interval[lvl - 1] - diff;
        	  tv.tv_usec = 0;
        	  area->pTimerLspRefresh[lvl - 1] = taskTimerSet(lspL1Refresh, tv, 0, (void *)area);
          }
          else if (lvl == IS_LEVEL_2)
          {
        	  tv.tv_sec = area->lsp_gen_interval[lvl - 1] - diff;
        	  tv.tv_usec = 0;
        	  area->pTimerLspRefresh[lvl - 1] = taskTimerSet(lspL2Refresh, tv, 0, (void *)area);
          }
        }
      else
        {
...
lsp_generate_pseudo (struct isis_circuit *circuit, int level)
{
  ...
  if (level == IS_LEVEL_1)
  {
	  circuit->u.bc.pTimerRefreshPseudoLsp[level - 1] = taskTimerSet(lspL1RefreshPseudo, tv, 0, (void *)circuit);
  }
  else if (level == IS_LEVEL_2)
  {
	  circuit->u.bc.pTimerRefreshPseudoLsp[level - 1] = taskTimerSet(lspL2RefreshPseudo, tv, 0, (void *)circuit);
  }
 ...
 */
void
lspL1RefreshPseudo (Int32T fd, Int16T event, void *args)
{
  struct isis_circuit *circuit;
  u_char id[ISIS_SYS_ID_LEN + 2];

  circuit = (struct isis_circuit *)args;
...
void
lspL2RefreshPseudo (Int32T fd, Int16T event, void *args)
{
  struct isis_circuit *circuit;
  u_char id[ISIS_SYS_ID_LEN + 2];
  ...
  for (lvl = IS_LEVEL_1; lvl <= IS_LEVEL_2; lvl++)
    {
       ...
       * Throttle avoidance
       */
      if(circuit->u.bc.pTimerRefreshPseudoLsp[lvl - 1])
      {
    	  taskTimerDel(circuit->u.bc.pTimerRefreshPseudoLsp[lvl - 1]);
    	  circuit->u.bc.pTimerRefreshPseudoLsp[lvl - 1] = NULL;
      }
      diff = now - lsp->last_generated;
      if (diff < circuit->area->lsp_gen_interval[lvl - 1])
        {
          ...
          if (lvl == IS_LEVEL_1)
          {
            tv.tv_sec = circuit->area->lsp_gen_interval[lvl - 1] - diff;
            tv.tv_usec = 0;
            circuit->u.bc.pTimerRefreshPseudoLsp[lvl - 1] = taskTimerSet(lspL1RefreshPseudo, tv, 0, (void *)circuit);
          }
          else if (lvl == IS_LEVEL_2)
          {
        	  tv.tv_sec = circuit->area->lsp_gen_interval[lvl - 1] - diff;
        	  tv.tv_usec = 0;
        	  circuit->u.bc.pTimerRefreshPseudoLsp[lvl - 1] = taskTimerSet(lspL2RefreshPseudo, tv, 0, (void *)circuit);
          }
        }
      else
        {
 ...
 */
void
lspTick (Int32T fd, Int16T event, void * arg)
{
  struct isis_area *area;
  struct isis_circuit *circuit;
  struct isis_lsp *lsp;
              ...
              if (lsp->age_out == 0)
                {
                  ...
                  if (lsp->from_topology)
                	  taskTimerDel (lsp->pTimerLspTopRef);
#endif /* TOPOLOGY_GENERATE */
                  lsp_destroy (lsp);
                  lsp = NULL;
                          ...
                          if (! listnode_lookup (circuit->lsp_queue, lsp))
                            {
                              ...
                              taskTimerSet(sendLsp, tv, 0, circuit);
                            }
                        }
                    }
                }
...
#ifdef TOPOLOGY_GENERATE
static void
top_lsp_refresh (Int32T fd, Int16T event, void * arg)
{
  struct isis_lsp *lsp;
  u_int16_t rem_lifetime, refresh_time;
  struct timeval tv;

  lsp = (struct isis_lsp *)arg;
  assert (lsp);

  lsp->pTimerLspTopRef = NULL;
  ...
  tv.tv_sec = lsp->area->lsp_refresh[0];
  tv.tv_usec = 0;
  lsp->pTimerLspTopRef = taskTimerSet (top_lsp_refresh, tv, 0, (void *)lsp);
...
generate_topology_lsps (struct isis_area *area)
{
	...
	for (i = 1; i < (max + 1); i++) {
		...
		tv.tv_sec = lsp->area->lsp_refresh[0];
		tv.tv_usec = 0;
		lsp->pTimerLspTopRef = taskTimerSet(top_lsp_refresh, tv, 0,
				(void *) lsp);
