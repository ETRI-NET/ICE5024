...
int team_init(struct team_handle *th, uint32_t ifindex)
{
	...
	if (th->family < 0) {
		err(th, "Failed to resolve netlink family. (err=%d)", th->family);
...
TEAM_EXPORT
int team_ioctl_link_enaslave(struct team_ifinfo *ifinfo, uint32_t port_ifindex)
{
	struct ifreq ifr;
	int	fd;
	int	err;

	strcpy(ifr.ifr_name , team_get_ifinfo_ifname(ifinfo));
	ifr.ifr_ifindex = port_ifindex;
	fd = socket(AF_INET, SOCK_DGRAM, 0);
	if (fd < 0)
		return errno;
	err = ioctl(fd, SIOCSTEAMADDSLAVE, &ifr);
	if(err) {
		close (fd);
		return err;
	}
...
TEAM_EXPORT
int team_ioctl_link_release(struct team_ifinfo *ifinfo, uint32_t port_ifindex)
{
	struct ifreq ifr;
	int	fd;
	int	err;
	...
	if (fd < 0)
		return errno;
	err = ioctl(fd, SIOCSTEAMDELSLAVE, &ifr);
	if(err) {
		close (fd);
		return err;
	}
...
TEAM_EXPORT
int team_ioctl_port_enabled(struct team_ifinfo *ifinfo, uint32_t port_ifindex, bool val)
{
	struct ifreq ifr;
	int	fd;
	int	err;
	...
	if (fd < 0)
		return errno;
	err = ioctl(fd, val == true ? SIOCSTEAMENAPORT : SIOCSTEAMDISPORT, &ifr);
	if(err) {
#ifdef	N2OS_KERNEL_2_6_3x
		if (errno == ENODEV) {
			err = 0;
...
TEAM_EXPORT
int team_ioctl_port_enabled_get(struct team_ifinfo *ifinfo, uint32_t port_ifindex, bool *enabled)
{
	struct ifreq ifr;
	int	fd;
	int	err;
	...
	if (fd < 0)
		return errno;
	err = ioctl(fd, SIOCGTEAMGETPORT, &ifr);
	if(err) {
#ifdef	N2OS_KERNEL_2_6_3x
		if (errno == ENODEV) {
			*enabled = false;
	...
	*enabled = ifr.ifr_ifru.ifru_ivalue ? true : false;

	return 0;
}

TEAM_EXPORT
int team_ioctl_port_user_linkup(struct team_ifinfo *ifinfo, uint32_t port_ifindex, bool val)
{
	struct ifreq ifr;
	int	fd;
	int	err;
	...
	if (fd < 0)
		return errno;
	err = ioctl(fd, val == true ? SIOCSTEAMENAUSER : SIOCSTEAMDISUSER, &ifr);
	if(err) {
		close (fd);
		return err;
	}
...
TEAM_EXPORT
int team_ioctl_port_user_linkup_get(struct team_ifinfo *ifinfo, uint32_t port_ifindex, bool *user_linkup)
{
	struct ifreq ifr;
	int	fd;
	int	err;
	...
	if (fd < 0)
		return errno;
	err = ioctl(fd, SIOCGTEAMGETUSER, &ifr);
	if(err) {
		close (fd);
		return err;
	}
	...
	*user_linkup = ifr.ifr_ifru.ifru_ivalue ? true : false;
...
TEAM_EXPORT
int team_ioctl_port_priority(struct team_ifinfo *ifinfo, uint32_t port_ifindex, int32_t priority)
{
	struct ifr_port_priority port_priority;
	struct ifreq ifr;
	int	fd;
	int	err;

	strcpy(ifr.ifr_name , team_get_ifinfo_ifname(ifinfo));
	ifr.ifr_data = &port_priority;
	port_priority.ifindex = port_ifindex;
	port_priority.priority = priority;
	fd = socket(AF_INET, SOCK_DGRAM, 0);
	if (fd < 0)
		return errno;
	err = ioctl(fd, SIOCSTEAMPORTPRIO, &ifr);
	if(err) {
		close (fd);
		return err;
	}
...
TEAM_EXPORT
int team_ioctl_port_priority_get(struct team_ifinfo *ifinfo, uint32_t port_ifindex, int32_t *priority)
{
	struct ifr_port_priority port_priority;
	struct ifreq ifr;
	int	fd;
	...
	ifr.ifr_data = &port_priority;
	port_priority.ifindex = port_ifindex;
	port_priority.priority = -1;
	fd = socket(AF_INET, SOCK_DGRAM, 0);
	if (fd < 0)
		return errno;
	err = ioctl(fd, SIOCGTEAMPORTPRIO, &ifr);
	if(err) {
		close (fd);
		return err;
	}
	...
	*priority = port_priority.priority;
...
#ifdef	N2OS
TEAM_EXPORT
int team_ioctl_port_updown(const char *ifname, bool up, bool *state)
{
	struct ifreq ifr;
	int	fd;
	int	err;
...
out:
	...
	if (ifr.ifr_flags & IFF_UP)	//current state
		*state = true;
	else
		*state = false;
...
TEAM_EXPORT
int team_ioctl_port_promiscuous(const char *ifname, bool promisc)
{
	struct ifreq ifr;
	int	fd;
	int	err;
...
TEAM_EXPORT
int team_ioctl_port_link_get(const char *ifname, uint32_t *port_ifindex)
{
	struct ifreq ifr;
	int	fd;
	int	err;
...
{
#ifdef	N2OS_KERNEL_2_6_3x
	return team_ioctl_port_enabled(th->ifinfo, port_ifindex, val);
#else
	struct team_option *option;
...
{
#ifdef	N2OS_KERNEL_2_6_3x
	return team_ioctl_port_user_linkup_get(th->ifinfo, port_ifindex, linkup);
#else
	struct team_option *option;
...
{
#ifdef	N2OS_KERNEL_2_6_3x
	return team_ioctl_port_user_linkup(th->ifinfo, port_ifindex, linkup);
#else
	struct team_option *option;
...
{
#ifdef	N2OS_KERNEL_2_6_3x
	return team_ioctl_port_priority_get(th->ifinfo, port_ifindex, priority);
#else
	struct team_option *option;
...
{
#ifdef	N2OS_KERNEL_2_6_3x
	return team_ioctl_port_priority(th->ifinfo, port_ifindex, priority);
#else
	struct team_option *option;
...
uint32_t team_ifname2ifindex(struct team_handle *th, const char *ifname)
{
	...
	int err;
#ifdef	N2OS_KERNEL_2_6_3x
	err = team_ioctl_port_link_get(ifname, &ifindex);
	if (err) {
		return 0;
	}
#else
 ...
 **/
TEAM_EXPORT
int team_port_up(struct team_handle *th, const char *ifname, bool *state)
{
	int err;
	err = team_ioctl_port_updown(ifname, true, state);
	return -nl2syserr(err);
}
 ...
 **/
TEAM_EXPORT
int team_port_down(struct team_handle *th, const char *ifname, bool *state)
{
	int err;
	err = team_ioctl_port_updown(ifname, false, state);
	return -nl2syserr(err);
}

/**
 * team_port_set_promisc:
 * @th: libteam library context
 * @ifname: port interface name
 *
 ...
 **/
TEAM_EXPORT
int team_port_set_promiscuous(struct team_handle *th, const char *ifname)
{
	int err;
	err = team_ioctl_port_promiscuous(ifname, true);
	return -nl2syserr(err);
}

/**
 * team_port_clear_promisc:
 * @th: libteam library context
 * @ifname: port interface name
 *
 ...
 **/
TEAM_EXPORT
int team_port_clear_promiscuous(struct team_handle *th, const char *ifname)
{
	int err;
	err = team_ioctl_port_promiscuous(ifname, false);
	return -nl2syserr(err);
}
#endif
...
int team_port_add(struct team_handle *th, uint32_t port_ifindex)
{
	int err;
#ifdef	N2OS_KERNEL_2_6_3x
	err = team_ioctl_link_enaslave(th->ifinfo, port_ifindex);
#else
...
#ifdef	N2OS_KERNEL_2_6_3x
	err = team_ioctl_link_release(th->ifinfo, port_ifindex);
#else
...
errout:
	rtnl_link_put(link);
#ifdef	N2OS_IFLA_ADDRESS
	if (err == -EBUSY) {	//for MCS7830 and more
		if (trial == false) {
			name = team_ifindex2ifname(th, ifindex, ifname, sizeof(ifname));
			if (name) {
				//warn(th, "Retry HW_SET =%u", ifindex);
				err = team_port_down(th, ifname, &old_state);
				if (err)
					return err;
				trial = true;
				goto retry;
		...
		if (trial == true) {
			if (old_state == true) {
				//warn(th, "Success HW_SET =%u", ifindex);
				err= team_port_up(th, ifname, &old_state);
				if (err)
					return err;
			}
		}
