...
#ifdef	N2OS
		bool active;
#define		LACP_PORT_CFG_DFLT_ACTIVE true
#endif
	} cfg;
};
...
#ifdef	N2OS
	teamd_log_dbg("%s: %s port by updating", tdport->ifname, new_enabled_state == true ? "Enabling" : "Disabling");
#endif
				    ...
				    struct lacp_port *next_agg_lead)
{
	...
	if (err) teamd_log_info("%s 2 err=%d", __FUNCTION__, errno);
	if (err)
		return err;
	return 0;
}
...
static int __slow_addr_add_del(struct lacp_port *lacp_port, bool add)
{
	...
	if (ret == -1) {
		teamd_log_err("%s (%s) ioctl %s failed.",
...
{
#ifdef	N2OS
	__slow_addr_add_del(lacp_port, false);
	return 0;
#else
...
#ifdef	N2OS
	teamd_log_dbg ("%s: send LACPDU @ %lu",
		lacp_port->tdport->ifname, time(NULL));
#endif
...
static int lacpdu_recv(struct lacp_port *lacp_port)
{
...
#ifdef	N2OS
	teamd_log_dbg ("%s: recv LACPDU @ %lu",
		lacp_port->tdport->ifname, time(NULL));
#endif
	...
	//teamd_log_dbg("%s:call lacp_port_timeout_set.", __FUNCTION__);
	err = lacp_port_timeout_set(lacp_port, false);
	if (err) {
		return err;
	}
	//teamd_log_dbg("%s:%s: %s: %d.", __FUNCTION__, LACP_TIMEOUT_CB_NAME, lacp_port->tdport->ifname, lacp_port->lacp->cfg.fast_rate ?  LACP_PERIODIC_SHORT: LACP_PERIODIC_LONG);
	teamd_loop_callback_enable(lacp_port->ctx,
				   LACP_TIMEOUT_CB_NAME, lacp_port);
	return 0;
}
			     ...
			     struct teamd_port *tdport)
{
	...
	if (err) {
#ifdef	N2OS
		teamd_log_err("%s: Failed to set hardware address (%s, %d). ",
			      tdport->ifname,
					strerror(-err), err);
#else
...
#ifdef	N2OS
	err = teamd_config_bool_get(ctx, &lacp_port->cfg.active,
				    "$.ports.%s.active", port_name);
	if (err)
		lacp_port->cfg.active = LACP_PORT_CFG_DFLT_ACTIVE;
	teamd_log_dbg("%s: Using active \"%d\".", port_name,
		      lacp_port->cfg.active);
#endif
	return 0;
}
...
#ifdef	N2OS
	teamd_log_dbg("%s: Disabling port by adding", tdport->ifname);
#endif
...
#ifdef	N2OS
static int lacp_port_state_active_get(struct teamd_context *ctx,
				    struct team_state_gsc *gsc,
				    void *priv)
{
	gsc->data.bool_val = lacp_port_gsc(gsc, priv)->cfg.active;
	return 0;
}
#endif
...
static const struct teamd_state_val lacp_port_state_vals[] = {
...
#ifdef	N2OS
	{
		.subpath = "active",
		.type = TEAMD_STATE_ITEM_TYPE_BOOL,
		.getter = lacp_port_state_active_get,
	},
#endif
	{
...
#ifdef	N2OS
static int lacp_modify(struct teamd_context *ctx, void *priv, struct teamd_port *tdport)
{
	int	err;
	struct lacp *lacp = priv;
	struct lacp_port *lacp_port;

	teamd_log_dbg("%s %p.", __FUNCTION__, ctx);
	if (ctx == NULL) {
		teamd_log_err("Failed to load config values: no context");
		return -1;
	}

	if (tdport == NULL) {
		struct teamd_port *tdport_restart;
		teamd_log_dbg("Modifying all %p.", ctx);
		err = lacp_load_config(ctx, lacp);
		if (err) {
			teamd_log_err("Failed to load config values.");
			return err;
		}
		teamd_for_each_tdport(tdport_restart, ctx) {
			lacp_port = lacp_port_get(lacp, tdport_restart);
			if (lacp_port) {
				teamd_log_dbg("%s modified.", tdport_restart->ifname);
				lacp_port_timeout_set(lacp_port, 0);
				lacp_port_periodic_set(lacp_port);
	...
	lacp_port = lacp_port_get(lacp, tdport);
	if (lacp_port == NULL) {
		teamd_log_err("%s: Failed to get port.", tdport->ifname);
		return -1;
	}
	err = lacp_port_load_config(ctx, lacp_port);
	if (err) {
		teamd_log_err("%s: Failed to load port config.", tdport->ifname);
		return err;
	}
	teamd_log_dbg("%s modified.", tdport->ifname);
	lacp_port_timeout_set(lacp_port, 0);
	lacp_port_periodic_set(lacp_port);
	return lacp_port_actor_update (lacp_port);
}
