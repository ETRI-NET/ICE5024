...
#ifdef	N2OS
static int teamd_ctl_method_port_modify(struct teamd_context *ctx,
					const struct teamd_ctl_method_ops *ops,
					void *ops_priv)
{
	const char *port_devname;
	...
	err = teamd_port_modify(ctx, port_devname);
	switch (err) {
	case -ENODEV:
		return ops->reply_err(ops_priv, "NoSuchDev", "No such device.");
	case 0:
		break;
	default:
		return ops->reply_err(ops_priv, "PortMOdifyFail", "Failed to del port.");
	}
	return ops->reply_succ(ops_priv, NULL);
}
#endif
...
static const struct teamd_ctl_method teamd_ctl_method_list[] = {
	...
	{
		.name = "PortModify",
		.func = teamd_ctl_method_port_modify,
