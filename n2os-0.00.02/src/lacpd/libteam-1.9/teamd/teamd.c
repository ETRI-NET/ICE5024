				       ...
				       struct teamd_context *ctx)
{
	...
	list_for_each_node_entry_safe(lcb, tmp, lcb_list, list) {
		for (i = 0; i < 3; i++) {
			...
			teamd_log_dbg("callback: %s, %p", lcb->name, lcb->priv);
				  ...
				  struct timespec *initial)
{
	...
	teamd_log_dbg("Set timer loop callback: %s, %p", cb_name, priv);
			       ...
			       void *priv)
{
	...
	if (!found)
		return -ENOENT;
	teamd_log_dbg("Enabled loop callback: %s, %p", cb_name, priv);
	teamd_run_loop_restart(ctx);
	return 0;
}
				...
				void *priv)
{
	...
	if (!found)
		return -ENOENT;
	teamd_log_dbg("Disabled loop callback: %s, %p", cb_name, priv);
	teamd_run_loop_restart(ctx);
	return 0;
}
...
#ifdef	N2OS
int teamd_port_modify(struct teamd_context *ctx, const char *port_name)
{
	uint32_t ifindex;
	struct teamd_port *tdport;
	int err;
...
static int teamd_start(struct teamd_context *ctx)
{
	...
	teamd_log_info("Exiting... 0x%p", ctx);
