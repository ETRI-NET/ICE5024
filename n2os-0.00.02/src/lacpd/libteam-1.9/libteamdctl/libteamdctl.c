...
#ifdef	N2OS
/**
 * teamdctl_port_modify:
 * @tdc: libteamdctl library context
 * @port_devname: port device name
 *
 * Modify specified port from team.
 *
 * Returns: zero on success or negative number in case of an error.
 **/
TEAMDCTL_EXPORT
int teamdctl_port_modify(struct teamdctl *tdc, const char *port_devname)
{
	return cli_method_call(tdc, "PortModify", NULL, "s", port_devname);
}
#endif
